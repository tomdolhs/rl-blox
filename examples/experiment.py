...
class OneHotObsWrapper(ObservationWrapper):
    def __init__(self, env: gym.Env):
        ...
        assert isinstance(env.observation_space,
                          Discrete), "env.observation_space must be Discrete"
...
def run_experiment(config: dict):
    ...
    logger.define_experiment(
        env_name=env_name, algorithm_name=f"AMT-{backbone}-{task_selector}", hparams=config)

    maps = [generate_random_map(size=4, p=prob, seed=seed)
            for prob in map_probs]
    ...
    train_set = DiscreteTaskSet(
        base_env=base_env, set_context=set_context_fn, contexts=contexts, context_aware=True)

    env_dummy = train_set.get_task(0)
    q_net = MLP(env_dummy.observation_space.shape[0], int(
        env_dummy.action_space.n), activation="relu", hidden_nodes=[128, 128, 128], rngs=nnx.Rngs(seed))
    q_target_net = nnx.clone(q_net)
    replay_buffer = MultiTaskReplayBuffer(ReplayBuffer(
        buffer_size=100_000, discrete_actions=True), len(train_set))
    optimizer = nnx.Optimizer(q_net, optax.adam(0.003), wrt=nnx.Param)
    train_st = partial(train_ddqn, q_net=q_net,
                       optimizer=optimizer, q_target_net=q_target_net)
...
if __name__ == "__main__":
    ...
    print(
        f"Start experiments: {', '.join([cfg['name'] for cfg in experiment_configs])}")
